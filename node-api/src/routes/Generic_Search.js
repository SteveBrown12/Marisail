...
search_Router.get(
  ...
  async (request, response) => {
    try {
      ...
      const where_Sql = build_Where_Clause(
      ...
      ${where_Sql} 
      ORDER BY \`${service_config.main_table}\`.\`${
        service_config.primary_key
      }\` DESC
      LIMIT 50 OFFSET 0
    `
        .trim()
      ...
      SELECT COUNT(*) as totalCount FROM \`${
        service_config.main_table
      }\`  ${build_Joins(service_config)}   ${where_Sql}`
        .trim()
        .replace(/\s+/g, ' ');
...
search_Router.get(
  ...
  async (request, response) => {
    ...
    `;
      ...
      if (
        (flat_Row[primary_Key] === null ||
          flat_Row[primary_Key] === undefined) &&
        flat_Row[safe_Key] != null
      ) {
      ...
      (service_config.tables || []).forEach((table) => {
        ...
        const collumns = Array.isArray(table.columns)
          ? table.columns
