...
const validate_Schema = async (config) => {
  ...
  try {
    ...
    const [main_table_check] = await connection.query(`SHOW TABLES LIKE ?`, [
      config.main_table,
    ]);
    if (main_table_check.length === 0) {
      errors.push(
        `Configuration error: The main table '${config.main_table}' does not exist in the database.`
      );
    }
    ...
    if (config.join_tables && config.join_tables.length > 0) {
      for (const table of config.join_tables) {
        const [join_table_check] = await connection.query(
          `SHOW TABLES LIKE ?`,
          [table]
        );
        if (join_table_check.length === 0) {
          errors.push(
            `Configuration error: The join table '${table}' does not exist in the database.`
          );
        }
      }
    }
  ...
  console.log(
    `✅ Schema validated successfully for service: ${config.schema_name}`
  );
};

export const join_table_check = async (serviceConfig) => {
  if (
    !serviceConfig?.join_tables ||
    !Array.isArray(serviceConfig.join_tables)
  ) {
    // No join_tables defined, or not an array — nothing to check
    return;
  }
  ...
  try {
    const errors = [];
    for (const joinTable of serviceConfig.join_tables) {
      const [result] = await connection.query('SHOW TABLES LIKE ?', [
        joinTable,
      ]);
      if (result.length === 0) {
        errors.push(
          `Join table '${joinTable}' does not exist in the database.`
        );
      }
    }
...
export const initialize_service = async (req, res, next) => {
  try {
    ...
    handle_error_response(
      res,
      `Service initialization failed: ${err.message}`,
      500
    );
  }
};
